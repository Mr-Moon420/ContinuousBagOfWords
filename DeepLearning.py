...
def tensorSum(t):
    if is1D(t):
        ...
        return sum(tensorSum(i) for i in t)
...
class Embedding(Layer):
    def __init__(self, num_embeddings, embedding_dims):
        self.num_embeddings = embedding_dims
        self.embedding_dims = embedding_dims

        self.embeddings = np.array([[random.random() for _ in range(embedding_dims)] for _ in range(num_embeddings)])
        self.grad = self.embeddings*0
    ...
    def backward(self, gradient):
        if self.last_input_id is not None:
            zero_row = np.array([0 for _ in range(self.num_embeddings)])
...
class WordEmbedding(Embedding):
    def __init__(self, vocab, embedding_dims):
        super().__init__(vocab.size(), embedding_dims)

        self.vocab = vocab

    def get_vector(self, word):
        word_id = self.vocab.getID(word)
        return self.embeddings[word_id]
    
class MultiWordEmbedding(WordEmbedding):
    def __init__(self, vocab, embedding_dims):
        super().__init__(vocab, embedding_dims)
    ...
    def forward(self, input):
        ...
        f = np.array([self.embeddings[i] for i in input])
        return f
    
    def backward(self, gradient):
        if self.last_inputs is not None:
            zeroRow = np.array([0 for i in range(self.embedding_dims)])
            for i in self.last_inputs:
                self.grad[i] = zeroRow

        self.last_inputs = self.input_ids
...
class SoftMaxCrossEntropy(Loss):
    ...
    def loss(self, predicted, actual):
        ...
        likelihoods = np.log(probs + 1e-30)*actual
